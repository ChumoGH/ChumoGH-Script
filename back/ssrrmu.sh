...
export PATH
Block="/etc/bin" && [[ ! -d ${Block} ]] && mkdir ${Block} 
Block > /dev/null 2>&1
...
echo -e "$BARRA"
	stty erase '^H' && read -p "(Predeterminado: chumogh):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="chumogh"
...
echo -e "$BARRA"
	stty erase '^H' && read -p "(Predeterminado: chumogh):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="chumogh"
...
clear
	echo -e "  ${blue_font_prefix}ShadowsocksR Manager NEW ADMIN-ChumoGH${Font_color_suffix}  ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
${BARRA1}
...
#source <(curl -sL https://raw.githubusercontent.com/casitadelterror/dankeladm/master/new/ssrrmu.sh)
#curl https://raw.githubusercontent.com/casitadelterror/dankeladm/master/new/ssrrmu.sh > /tmp/ssrrmu.sh
#sed -i "s;DANKELTHAHER;ChumoGH;g" /tmp/ssrrmu.sh
#sed -i "s;dankel;chumogh;g" /tmp/ssrrmu.sh
#chmod +x /tmp/ssrrmu.sh
#bash /tmp/ssrrmu.sh
